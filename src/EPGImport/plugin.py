...
class EPGImportConfig(ConfigListScreen, Screen):
    ...
    def doimport(self, one_source=None):
        ...
        print("[EPGImport] Config loaded:", cfg)
        # Evita il KeyError
        if "sources" not in cfg:
            print("[EPGImport] ERROR: 'sources' key not found! Initializing as empty list.")
            cfg["sources"] = []
...
class EPGImportSources(Screen):
    ...
    def do_reset(self):
        ...
        self.epg = epgdb.epgdb_class(provider_name, provider_priority, epgdbfile)
...
class AutoStartTimer:
    ...
    def onTimer(self):
        self.autoStartImport.stop()
        now = int(time())
        log.write("[EPGImport] onTimer occured at %s\n" % strftime("%Y-%m-%d %H:%M:%S", localtime(now)))
        wake = self.getWakeTime()
    ...
    def startStandby(self):
        if Screens.Standby.inStandby:
            ...
            try:
                ...
                print("[EPGImport] error inStandby.onClose append .onLeaveStandby")
