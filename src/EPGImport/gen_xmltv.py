...
class Gen_Xmltv():
    def iterator(self, fd, channelsDict, offset=0):
        try:
            ...
            # Prova a iterare sul file
            for r in xmltv_parser.enumFile(fd):
                yield r
                
        except (LZMAError, IOError) as e:
            print("[gen_xmltv] Compression/IO Error: " + str(e))
            print("[gen_xmltv] File is corrupted or uses unsupported compression")
            # Esci silenziosamente senza yield
            return
        except Exception as e:
            print("[gen_xmltv] General Error: " + str(e))
            import traceback
            traceback.print_exc()
